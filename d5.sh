...
GENESIS_SEEDS=(
  "snoPBrXtMeMyMHUVTgbuBh1VycTnMH8VbXiXbM3SgLx9GWwp9c1w"
...
USER_SEED=$(echo "$USER_JSON" | python3 -c "import json,sys; print(json.load(sys.stdin)['result']['seed'])" 2>/dev/null)
...
ATK_SEED=$(echo "$ATK_JSON" | python3 -c "import json,sys; print(json.load(sys.stdin)['result']['seed'])" 2>/dev/null)
...
# --- patch forger: add xrp-pay-d5 case
python3 - <<'PYEOF'
import sys
src = open("$WS/repo/pocforger/forger_test.go".replace("$WS","__WS__"), encoding="utf-8").read() if False else open("${WS}/repo/pocforger/forger_test.go".replace("${WS}","__WS__") if False else None, encoding="utf-8").read() if False else None
PYEOF

# (simpler approach: write the patch directly with shell variables expanded)
python3 - "$WS" <<'PYEOF2'
import sys, os
ws = sys.argv[1]
path = os.path.join(ws, "repo", "pocforger", "forger_test.go")
src = open(path, encoding="utf-8").read()

NEW_CASE = '''	case "xrp-pay-d5":
        ...
        amount, ok := new(big.Int).SetString(d5.Amount, 10)
...
ANCHOR = '\tcase "xrp-pay":'
if NEW_CASE.count(ANCHOR.split('\n')[-1]) > 0 and ANCHOR in src:
    if 'xrp-pay-d5' in src:
        print("forger already patched (xrp-pay-d5 present)")
    else:
        src = src.replace(ANCHOR, NEW_CASE, 1)
        open(path, "w", encoding="utf-8", newline="\n").write(src)
        print("forger patched: xrp-pay-d5 case added")
elif 'xrp-pay-d5' in src:
    print("forger already patched (xrp-pay-d5 present)")
else:
    sys.exit("ERROR: anchor not found in forger_test.go")
PYEOF2
[ $? -ne 0 ] && { fail "forger patch failed"; exit 1; }
info "forger patched (xrp-pay-d5)"
